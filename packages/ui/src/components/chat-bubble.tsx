...
export function ChatBubble({ children, time, className }: ChatBubbleProps) {
  return (
    <div className={cn("rounded-2xl border-[2.5px] border-ink bg-chat-bg p-2.5", className)}>
      <div className="whitespace-pre-wrap wrap-break-word rounded-xl rounded-br-sm border-2 border-ink bg-chat px-3 py-2.5 font-mono text-[15px] leading-relaxed motion-safe:animate-pop">
        {children}
      </div>
      {time ? <p className="mt-1 text-right text-[11px] font-extrabold text-wa-dark">✓✓ {time}</p> : null}
    </div>
  );
}
